...
static uint8_t patterns[] = {0b10101010,0b01010101,0b11001100,0b00110011,0b11110000,0b11110000};
...
int main(void)
{
  ...
  while (i <= 5){
 	  uint8_t num = patterns[i];

 	  write_to_address(i, num);
 	  i++;
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  //checking if the button PA0 is pressed
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 0){
		  //If it is pressed, change between states
		  if (state==0)
		  {
			  //Changing to a half a second delay
			  state = 1;
			  TIM16 -> ARR = 499;
		  }
		  else{
			  //Changing to a 1 second delay
			  state = 0;
			  TIM16 ->ARR = 999;
		  }
	  }
...
void TIM16_IRQHandler(void)
{
	...
	//Keeping track of the array and resetting it when the pattern is over
	if (count > 5){count = 0; }

	pattern = read_from_address(count);
	//Checking whether the value has been read correctly
	if (pattern == patterns[count])
	{
		GPIOB ->ODR = pattern;
		count ++;
	}
	//Indicating SPI failure
	else{ GPIOB ->ODR = 0b00000001;}
